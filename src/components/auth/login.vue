<template>
	<div>
		<vs-dialog not-close :loading="loading" @close="back" :prevent-close="true" :blur="true" v-model="aa">
			<template #header>
			...
			<template #footer>
				<div class="footer-dialog">
					...
					<div class="new">
						{{ $t('login.not_account') }}
						<router-link :to="'/front/auth/register'">
							<h5 class="primary-text">{{ $t('login.forgot_password') }}</h5>
						</router-link>
					</div>
				</div>
			</template>
